...
const W4 = ({
   ...
   b0 = [{
      ...
      fullContent: `
         ...
         <p><strong>Boundary conditions (B.C.)</strong> are constraints necessary for the solution of a boundary value problem. A boundary value problem is a differential equation (or system of differential equations) to be solved in a domain where conditions are known on its boundary.</p>

         <p>Boundary value problems are extremely important as they model a vast array of phenomena and applications—from solid mechanics to heat transfer, and from fluid mechanics to acoustic diffusion. They arise naturally in every problem based on a differential equation to be solved in space, while initial value problems usually refer to problems to be solved in time.</p>

         <p>Each point in space can translate in three directions and rotate around three axes. We can then restrain movement in any of those directions based on how the model behaves in reality.</p>

         <p>In this blog, our main intention is to understand how boundary conditions can be applied in FEniCS. They may be specified in a number of ways.</p>

         <p>The simplest approach is to specify the boundary by a <code>SubDomain</code> object, using the <code>inside()</code> function to indicate which facets the boundary conditions should be applied to. The boundary facets will be searched for and marked <em>only</em> on the first call to <code>apply</code>. This means that the mesh can be moved after the first application, and the boundary markers would still remain intact.</p>

         <p>Alternatively, the boundary may be specified by a <code>MeshFunction</code> over facets, labeling all mesh facets with a number that specifies which ones should be included in the boundary.</p>
      `,
