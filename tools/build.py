...
def collaboration_strip():
    items = PROFILE.get('collaborators', [])
    if not items:
        return ''
...
def home():
    ...
    roles = ''.join(f'<a class="role-card" href="{e(href(r))}"><img class="role-sketch" src="{e(asset("assets/sketches/"+r.rsplit("/",1)[-1]+".png"))}" alt="" width="1536" height="1024" loading="lazy"><div class="role-caption"><span class="role-title">{e(t)}</span></div></a>' for r,t,_ in ROLES)
    ...
    selected = [p for p in [next((p for p in PAGES if p.get('record_type')==kind),None) for kind in ['publication','research']] if p]
    featured = ''.join('<a class="home-record illustrated-record focus-slide" data-focus-slide aria-label="Slide '+str(i+1)+' of '+str(len(selected))+'" aria-hidden="'+str(i != 0).lower()+'"'+(' tabindex="-1"' if i else '')+' href="'+e(href(p['route']))+'"><div class="feature-sketch-area"><img class="feature-sketch" src="'+e(asset('assets/sketches/focus-'+p['record_type']+'.png'))+'" width="1536" height="1024" alt="" loading="lazy"></div><div class="home-record-copy"><span class="profile-kicker accent">'+e(' · '.join(x for x in [p.get('date',''),p['section']] if x))+'</span><h3>'+e(p['title'])+'</h3><p>'+e(p['summary'])+'</p><span class="text-link">Read record</span></div></a>' for i,p in enumerate(selected))
    focus_carousel = f'''<div class="focus-carousel" data-focus-carousel aria-roledescription="carousel" aria-label="Research in focus"><div class="focus-viewport"><div class="focus-track" data-focus-track>{featured}</div></div><div class="focus-controls"><button type="button" class="focus-control" data-focus-prev aria-label="Show previous research item">←</button><span data-focus-status>1 / {len(selected)}</span><button type="button" class="focus-control" data-focus-next aria-label="Show next research item">→</button></div></div>'''
    news = home_news()
