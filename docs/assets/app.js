  ...
  function buildMegaMenus(config) {
    for (const group of config.menus) {
      ...
      for (const item of group.features) {
        ...
        const img = el('img', ''); img.src = new URL(item.image, assetBase).href; img.alt = ''; img.width = 288; img.height = 162; img.loading = 'lazy';
        const play = el('span', 'mega-play'); play.setAttribute('aria-hidden', 'true');
        thumbnail.append(img, play);
        const copy = el('div', 'mega-feature-copy'); const action = el('span', 'mega-watch');
        action.append(navIcon(item.action === 'Read tutorial' ? 'open-book' : 'play'), document.createTextNode(item.action || 'Watch video'));
